...
const agentMeta = [
  ...
  { key: "financialAnalysis", name: "Financial Analysis", icon: TrendingUp, color: "text-blue-400", bgColor: "bg-blue-500/10", borderColor: "border-blue-500/30" },
...
export default function AgentAnalysisPage() {
  ...
  const handleRunAnalysis = async () => {
    ...
    try {
      ...
      if (res.data.success) {
        setResults(res.data.agentResults);
          ...
          {agentMeta.map((agent, idx) => {
            ...
            return (
              <motion.div
                ...
                className={`rounded-2xl bg-[#080d1a] border overflow-hidden shadow-xl transition-all ${status === "complete" ? "border-brand/30" : status === "running" ? "border-blue-500/30" : "border-[#1e293b]"
                  }`}
              >
                <div className="p-6">
        ...
        {running && (
          <div className="p-8 rounded-2xl bg-[#0f172a]/80 border border-[#1e293b] relative overflow-hidden shadow-xl">
            ...
            <div className="relative z-10 flex flex-col gap-4">
              ...
              <div className="font-mono text-xs md:text-sm text-blue-400/90 space-y-2 mt-2">
                ...
                <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 6 }} className="text-blue-300">[SYS] Awaiting Groq AI responses for all agents...</motion.p>
                <p className="animate-pulse mt-4 text-blue-500">_</p>
              </div>
            </div>
          </div>
        )}
